...
import { FiUsers, FiCalendar, FiArrowRight, FiCamera } from 'react-icons/fi';
...
const CommunityProfile = () => {
  ...
  const { community, memberCount, isMember, isAdmin, membershipStatus, membersPreview, events } = profile;
      ...
      <div className="max-w-3xl mx-auto space-y-4">
        ...
        <div className={`rounded-3xl overflow-hidden border ${isSpace ? 'border-white/10 bg-white/5' : 'border-uv-border bg-white'}`}>
          <div className="relative h-44 md:h-56">
            {community.cover_url ? (
              <img src={`http://localhost:3000${community.cover_url}`} className="w-full h-full object-cover" alt={community.community_name} />
            ) : (
            ...
            <div className="absolute left-4 bottom-3 flex items-center gap-3">
              ...
              >
                ...
                {community.avatar_url ? (
                  <img
                    src={`http://localhost:3000${community.avatar_url}`}
        ...
        {/* Jobs removed from community view (staff only) */}
