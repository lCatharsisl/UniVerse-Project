...
const EventApplicationForm = () => {
  ...
  const { eventApplicationId } = useParams<{ eventApplicationId: string }>();
  ...
  const load = async () => {
    if (!eventApplicationId) return;
    setLoading(true);
    setError('');
    try {
      const res = await api.get(`/community/events/applications/${eventApplicationId}`);
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.patch(`/community/events/applications/${eventApplicationId}`, form, {
  ...
  const cancel = async () => {
    ...
    setCancelError('');
    try {
      await api.patch(`/community/events/applications/${eventApplicationId}/cancel`, { note: 'Cancelled by applicant' });
      navigate(`/community/${community?.community_id}`);
    } catch (e: any) {
  ...
  if (error || !application || !event || !community) {
    return (
      <div className={`min-h-screen p-4 md:p-6 ${isSpace ? 'bg-[#050510]' : 'bg-white'}`}>
    ...
    <div className={`min-h-screen p-4 md:p-6 ${isSpace ? 'bg-[#050510]' : 'bg-white'}`}>
      <div className="max-w-3xl mx-auto space-y-4">
        ...
        <div className={`rounded-3xl border ${isSpace ? 'border-white/10 bg-white/5' : 'border-uv-border bg-gray-50'} p-4 md:p-5`}>
          <h1 className={`text-2xl font-black ${isSpace ? 'text-white' : 'text-uv-black'}`}>{t('communityApplication.eventTitle')}</h1>
          <p className={`text-sm mt-2 ${isSpace ? 'text-white/70' : 'text-uv-gray'}`}>
            <span className="font-black">{community.community_name}</span> · {event.title}
          </p>
