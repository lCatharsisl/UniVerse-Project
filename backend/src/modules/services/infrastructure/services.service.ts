...
export class RemoteServicesService {
  static async createLostItem(
    ...
    const lostDate = data.lostDate ? new Date(data.lostDate) : new Date();
    ...
    return transaction(async (client) => {
      ...
      if (result.rows.length === 0) {
        throw AppError.internal('Failed to create lost item');
      }
  ...
  static async createFoundItem(
    ...
    const foundDate = data.foundDate ? new Date(data.foundDate) : new Date();
    ...
    return transaction(async (client) => {
      ...
      if (result.rows.length === 0) {
        throw AppError.internal('Failed to create found item');
      }
  ...
  static async getLostItems(queryParams: any) {
    const { location, isResolved, startDate, endDate, limit = 50, offset = 0 } = queryParams;
  ...
  static async getFoundItems(queryParams: any) {
    const { location, isResolved, startDate, endDate, limit = 50, offset = 0 } = queryParams;
  ...
  static async resolveLostItem(itemId: number, resolvedByUserId: number): Promise<void> {
    ...
    if (item.user_id !== resolvedByUserId) throw AppError.forbidden('You can only resolve your own items');
    if (item.is_resolved) throw AppError.badRequest('Item is already resolved');
  ...
  static async addComment(userId: number, itemType: 'lost' | 'found', itemId: number, content: string): Promise<void> {
    await query(
  ...
  static async getItemImages(itemType: 'lost' | 'found', itemId: number): Promise<string[]> {
  ...
  static async updateLostItem(itemId: number, userId: number, data: any): Promise<void> {
    const item = await queryOne<any>('SELECT * FROM lost_items WHERE lost_item_id = $1', [itemId]);
    if (!item) throw AppError.notFound('Lost item not found');
    if (item.user_id !== userId) throw AppError.forbidden('You can only edit your own items');

    await query(
      `UPDATE lost_items
       SET lost_item_name = COALESCE($1, lost_item_name),
           location = COALESCE($2, location),
           description = COALESCE($3, description)
       WHERE lost_item_id = $4`,
      [data.lostItemName || null, data.location || null, data.description || null, itemId]
    );
  }

  static async updateFoundItem(itemId: number, userId: number, data: any): Promise<void> {
    ...
    await query(
      `UPDATE found_items
       SET found_item_name = COALESCE($1, found_item_name),
           location = COALESCE($2, location),
           description = COALESCE($3, description)
       WHERE found_item_id = $4`,
      [data.foundItemName || null, data.location || null, data.description || null, itemId]
    );
  }
  ...
  static async deleteLostItem(itemId: number, userId: number): Promise<void> {
    ...
    await transaction(async (client) => {
      await client.query('DELETE FROM item_comments WHERE item_type = $1 AND item_id = $2', ['lost', itemId]);
      await client.query('DELETE FROM lost_item_images WHERE lost_item_id = $1', [itemId]);
      await client.query('DELETE FROM lost_items WHERE lost_item_id = $1', [itemId]);
    });
  }
  ...
  static async deleteFoundItem(itemId: number, userId: number): Promise<void> {
    ...
    // Hard delete
    await transaction(async (client) => {
      await client.query('DELETE FROM item_comments WHERE item_type = $1 AND item_id = $2', ['found', itemId]);
      await client.query('DELETE FROM found_item_images WHERE found_item_id = $1', [itemId]);
      await client.query('DELETE FROM found_items WHERE found_item_id = $1', [itemId]);
    });
  }
}
